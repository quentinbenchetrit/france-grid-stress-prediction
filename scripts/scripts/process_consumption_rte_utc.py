...
def _to_utc_from_local_naive(dt: pd.Series) -> pd.Series:
    """
    Interpret naive datetimes as Europe/Paris then convert to UTC.
    Robust DST handling:
      - ambiguous (fall-back repeated hour): set to NaT (then caller drops)
      - nonexistent (spring-forward missing hour): shift forward
    """
...
def tidy_year(df_raw: pd.DataFrame, year: int, step_minutes: int = 30) -> pd.DataFrame:
    ...
    Output (UTC-only): datetime, date, year, statut, slot_index, load_mw
    - datetime is tz-aware UTC
    - date is tz-aware UTC (midnight)
    """
    df = df_raw.copy()
    df.columns = [f"col_{i}" for i in range(df.shape[1])]

    # Parse day date from first column (naive date)
    ...
    # Build naive local datetime (Europe/Paris implied), then convert to UTC
    df_long["slot_index"] = df_long.groupby("date").cumcount()

    # naive local datetime
    dt_local_naive = df_long["date"] + pd.to_timedelta(df_long["slot_index"] * step_minutes, unit="m")

    # convert to tz-aware UTC
    df_long["datetime"] = _to_utc_from_local_naive(dt_local_naive)

    # Convert "date" as UTC midnight (kept for convenience / grouping)
    df_long["date"] = _to_utc_from_local_naive(df_long["date"])

    df_long["year"] = year
    df_long = df_long[["datetime", "date", "year", "statut", "slot_index", "load_mw"]]

    # If DST handling or raw quirks created duplicate UTC timestamps, aggregate safely
    if df_long["datetime"].duplicated().any():
        df_long = (
            df_long.groupby(["datetime", "date", "year", "statut", "slot_index"], as_index=False)["load_mw"]
            .mean()
        )
...
def process_year(raw_cons_dir: Path, out_dir: Path, year: int, step_minutes: int = 30) -> None:
    ...
    out_path = out_dir / f"consommation_{year}_long_utc.csv"
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Convert RTE consumption yearly files into long CSV format (UTC-only timestamps)."
    )
    p.add_argument(
        "--base-dir",
    ...
    p.add_argument(
        ...
        help="Output folder under base-dir.",
    )
    p.add_argument(
        "--years",
        default="1996-2025",
