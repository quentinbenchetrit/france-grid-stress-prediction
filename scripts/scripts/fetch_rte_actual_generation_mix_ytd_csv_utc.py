...
# Keep only these production types in the final CSV
...
def fetch_generation_mix_15min(
    ...
    # Keep behaviour identical to your script:
    if production_type:
...
def to_utc(series: pd.Series) -> pd.Series:
    ...
    and prevents mixed-tz object dtype issues.
    """
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fetch RTE Actual Generation (mix 15-min) YTD and save to CSV (SOLAR+WIND only, UTC-only timestamps)."
    )
    parser.add_argument(
        "--out",
        default="/home/onyxia/work/france-grid-stress-prediction/data/interim/production/rte_actual_generation_mix_15min_ytd_solar_wind_utc.csv",
        help="Output CSV file path (UTC timestamps).",
    )
    parser.add_argument(
        "--timezone",
        default=LOCAL_TZ,
        help="Timezone used for date parameters and 'start of year' (API calls).",
    )
    parser.add_argument(
        "--initial-days",
        type=int,
    ...
    print(f"Data coverage UTC: {df['start_date'].min()} -> {last_available}")
    return 0
